            ...
            <p class="text-sm text-gray-300 mt-1">
                {{
                    $registrations
                        ->filter(fn($reg) =>
                            $reg->teacher &&
                            $reg->submissions->count() === $workshop->assignments->count()
                        )
                        ->count()
                }}
                out of {{ $registrations->count() }} teachers have submitted all assignments.
            </p>
                    ...
                    <tbody>
                        @forelse ($registrations as $registration)
                            <tr class="border-t border-gray-600">
                                ...
                                <td class="px-4 py-2 {{ $registration->submissions->count() === $workshop->assignments->count() ? 'text-green-400 font-semibold' : 'text-white' }}">
